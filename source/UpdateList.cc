                    ...
                    const std::string url = "")
{
  ...
  for (unsigned int dln = 0; dln < no_downloads; ++dln) {
    ...
    // Apply translations ...
    char* cmd[] = {"sed", "-f", "misc/share/CVTranslations", 0};
    pstream sed ("sed", cmd);
    
    if (!url.empty())
      info.url = url;
    else if (!package.cv_url.value.empty())
...
int main (int argc, char* argv[])
{
  ...
  bool odd = true, noprefix = false, beta = false;
  std::string url;
  ++argv; --argc; // progname
  while (argc > 0) {
  ...
  for (int i = 0; i < argc; ++i)
	...
	if (x.begin() != x.end()) {
      ...
      Check4Updates (package, odd, noprefix, beta, url);
    }
}
