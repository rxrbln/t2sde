# --- T2-COPYRIGHT-BEGIN ---
# t2/package/*/stone/stone_gui_dialog.sh
...
gui_menu() {
	...
	cmd="gui_dialog --ok-label Select"
	if [ $id = main ]; then local cmd="$cmd --cancel-label Quit"
	elif [ "$gui_nocancel" = 1 ]; then cmd="$cmd --no-cancel"
	else local cmd="$cmd --cancel-label Continue"; fi
...
gui_input() {
	...
	cmd="gui_dialog"
	if [ "$gui_nocancel" = 1 ]; then cmd="$cmd --no-cancel"
	else local cmd="$cmd --cancel-label Continue"; fi
...
gui_message() {
	local headlines="$( echo "$1" | fmt -$gui_dialog_s62 )"
	gui_dialog --ok-button Continue --msgbox "$headlines" \
