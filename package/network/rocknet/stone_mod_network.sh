# --- T2-COPYRIGHT-BEGIN ---
# t2/package/*/rocknet/stone_mod_network.sh
...
set_name() {
	local old1="$HOSTNAME" old2="$HOSTNAME.$DOMAINNAME" old3="$DOMAINNAME"
	if [ $1 = HOSTNAME ]; then
		gui_input "Set a new hostname (without domain part)" "${!1}" "$1"
	else
		gui_input "Set a new domainname (without host part)" "${!1}" "$1"
	fi

	[ "${!1}" ] || return # user cancelled?

	local new="$HOSTNAME.$DOMAINNAME $HOSTNAME"
...
main() {
	...
	p_interfaces=$(ip -o link | sed -n '/link\/ether/ s/[^:]*: \([^:]*\):.*/\1/p')
...
rocknet is executed.'"
	...
	cmd="$cmd 'Static hostname:   ${HOSTNAME:-<auto>}'   'set_name HOSTNAME'"
