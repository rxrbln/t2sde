...
char env_PREVLEVEL[64]="PREVLEVEL=N";
char env_RUNLEVEL[64]="RUNLEVEL=N";
char env_TERM[64]="TERM=linux";
char env_INIT_HALT[32]="";
...
int main(int argc, char ** argv) {
	...
	if ( getenv("INIT_HALT") )
		sprintf(env_INIT_HALT, "INIT_HALT=%.16s", getenv("INIT_HALT"));
...
#ifdef __SIGRTMIN
		if ( i >= __SIGRTMIN ) continue;
#endif
#ifdef SIGUNUSED
		if ( i > SIGUNUSED ) continue;
#endif
